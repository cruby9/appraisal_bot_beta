...
def find_row_and_column_old(square_footage, quality_rating,file):
...
def find_basement_un_row_and_column(square_footage, quality_rating,df):
    # read the excel file into a pandas dataframe
...
def find_basement_fn_row_and_column(square_footage, quality_rating,df):
    # read the excel file into a pandas dataframe
...
def find_detached_garage_row_and_column(square_footage, quality_rating,df):
    # read the excel file into a pandas dataframe
...
def find_attached_garage_row_and_column(square_footage, quality_rating,df):
    # read the excel file into a pandas dataframe
...
def find_row_and_column(square_footage, quality_rating, file):
    # read the excel file into a pandas dataframe
    #df = pd.read_excel("costdocs/square_foot_table.xlsx", header=2, index_col=0)
    df = pd.read_excel(file, header=2, index_col=0)
    print("Dataframe:")
    print(df)
    ...
    for col in df.columns:
        left, right = col.split('-')
        output.append(f"Checking column {col}: {left} <= {square_footage} <= {right}")
        if int(left) <= square_footage <= int(right):
            column = col
            break
    ...
    row = None
    for i, q in enumerate(df.index):
        output.append(f"Checking row {q}: {q} == {quality_rating}")
        if q == quality_rating:
            row = i
            break
    ...
    # print the row and column numbers
    if row is not None and column is not None:
        output.append(f"Row {row+4}, Column {pd.Index(df.columns).get_loc(column)+2}")
        cell_value = df.loc[quality_rating, column]
        output.append(f"Cell value: {cell_value}")
        
        # compute the estimated construction value
        cost = square_footage * cell_value
        output.append(f"Estimated construction value: ${cost:,.2f}")
    else:
        output.append("Matching row or column not found")
        cost = None
    
    return output, row, column, cost
