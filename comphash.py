#this program is import the comp data to prepare
...
df = pd.read_csv('comparable.csv')

# Print the first 5 rows of the DataFrame
print(df.head())


df = pd.read_csv('comparable.csv', header=0)
...
def format_levels(levels, prop_sub_type, arch_style):
    ...
    levels_dict = {
        'One': f"DT1;{'Ranch' if pd.isna(arch_style) or arch_style == '' else arch_style}",
        'Two': f"DT2;{'2Story' if pd.isna(arch_style) or arch_style == '' else arch_style}",
        'Multi/Split': f"DT2;{'SplitLevel' if pd.isna(arch_style) or arch_style == '' else arch_style}",
        'Bi-Level': f"DT2;{'Bilevel' if pd.isna(arch_style) or arch_style == '' else arch_style}",
        'Three Or More': f"DT3;{'3Story' if pd.isna(arch_style) or arch_style == '' else arch_style}"
    }

    if pd.isna(levels) or levels == '':
        return levels_dict['One']
    else:
        return levels_dict.get(levels, '')        
...
new_df = pd.DataFrame({
    ...
    'Fireplaces': df['Fireplaces Total'].fillna(0)
})
